...
from app.send_mail import send_reset_password_email
...
def new_book():
    ...
    if form.validate_on_submit():
        book = Book(name=form.name.data, description=form.description.data,
                    country=form.country.data, price=form.price.data, author=form.author.data, genres=form.genres.data)
        db.session.add(book)
        db.session.commit()
        return redirect(url_for('index'))
    return render_template('new_book.html', form=form)
...
def save_book(book_id):
    ...
    db.session.commit()
    return '<h1>You have saved this tour successfully</h1>'
